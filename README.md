...
괄호로 묶인 토큰은 내부를 개별적으로 처리합니다. 예외에 해당하는 항목만 살리고 나머지는 제거한 뒤 재조합합니다.

```
detect_prefix: @
exceptions: @c, @f, @_@

입력: a, @b, (@c), @bc, (@d:0.5), (@e, @f:1.2), @_@
...
처리 과정:
  ...
  (@c)      → 내부 @c 가 예외 → 유지
  @bc       → 접두사 있음, 예외 아님 → 삭제
  (@d:0.5)  → 내부 @d 가 예외 아님 → 삭제
  (@e, @f:1.2) → 내부 개별 처리:
                   @e → 예외 아님 → 삭제
                   @f:1.2 → @f 가 예외 → (@f:1.2) 로 재조합
  @_@       → 예외 → 유지

결과: a, (@c), (@f:1.2), @_@
```

> `(@e, @f:1.2)` 처럼 여러 태그가 가중치를 공유하는 형태도 내부를 개별 처리합니다.
